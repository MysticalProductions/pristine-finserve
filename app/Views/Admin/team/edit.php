<div class="page-header">
  <h1>Edit Promoter Profile</h1>
  <a href="/admin/team" class="btn btn-outline btn-sm"><i class="fas fa-arrow-left"></i> Back to Promoter</a>
</div>

<div class="card">
  <div class="card-body">
    <form method="POST" action="/admin/team/update/<?= (int)($member['id'] ?? 0) ?>" enctype="multipart/form-data">
      <?= csrfField() ?>
      <div class="form-row">
      ...
      <?php if (!empty($member['photo'])): ?>
        <div style="margin-bottom:18px;">
          ...
          <img src="<?= uploadUrl(htmlspecialchars($member['photo'])) ?>" alt="" style="max-width:120px;border-radius:6px;border:1px solid var(--border);">
        </div>
      <?php endif; ?>

      <div class="form-row">
        <div class="form-group">
          <label for="photo"><?= !empty($member['photo']) ? 'Change Photo' : 'Photo' ?></label>
      ...
      <div style="display:flex;gap:10px;margin-top:24px;">
        <button type="submit" class="btn btn-gold"><i class="fas fa-save"></i> Update Promoter</button>
